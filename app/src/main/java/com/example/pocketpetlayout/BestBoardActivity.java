...
public class BestBoardActivity extends AppCompatActivity {
    public final static String TAG = "BestBoardActivity";
    ...
    //게시글 리스트
    ArrayList<BoardItem> bestBoardItems;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!bestBoardItems.isEmpty()) {
            //Listview 지정
            ListView bestListView = this.findViewById(R.id.bestListView);

            // ListView Adpater 지정
            final BoardAdapter boardAdapter = new BoardAdapter(this, bestBoardItems);

            // ListView의 어뎁터를 셋한다.
            bestListView.setAdapter(boardAdapter);

            //ListView 내부 아이템이 클릭 되었을 경우?
            bestListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
...
*/
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.toolbar_write:
                Toast.makeText(getApplicationContext(), "글 쓰기 버튼 클릭됨", Toast.LENGTH_LONG).show();
                return true;
            default:
                return false;
        }
    ...
    public void InitializeQnABoardData(){
        ...
        bestBoardItems = new ArrayList<BoardItem>();

        // board 테이블에서 좋아요수 10개 이상인 튜플의 id, title, writer, reg_date, like_cnt, com_cnt 를 가져온다.
        ...
        Cursor c = db.rawQuery("SELECT " +
                ...
                Board.COLUMN_LIKE_CNT + " >= 10 ORDER BY " + Board.COLUMN_BOARD_ID + " DESC; ", null );
        ...
        if (c.moveToFirst()) {
            ...
            do{
                ...
                int com = c.getInt(5);
                bestBoardItems.add(new BoardItem(id,title,writer,regDate,heart,com));
